import { API_FULL_URL, API_URL } from './const';
import { FullMovieType, ResponseType } from './types/types';

export const average = (arr: number[]) =>
  arr.reduce((acc, cur, i, arrs) => acc + cur / arrs.length, 0);

export const fetchMovies = async (query: string, controller: AbortController) => {
  const res = await fetch(`${API_URL}${query}`, {signal: controller.signal});
  if (!res.ok) {
    throw new Error('Could not load');
  }
  const data = await res.json() as ResponseType;
  if (!data.Search) {
    throw new Error('Films not found');
  }
  return data;
};

export const fetchFullMovie = async (id: string, controller: AbortController) => {
  const res = await fetch(API_FULL_URL.replace('__ID__', id), {signal: controller.signal});
  if (!res.ok) {
    throw new Error('Could not load');
  }
  return await res.json() as FullMovieType;
};
